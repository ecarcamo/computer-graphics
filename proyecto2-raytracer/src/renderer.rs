...
pub fn render<'a>(
    ...
    let top_positions = [
        (0,0,0),(1,0,0),(2,0,0),
        (0,0,1),(1,0,1),/*(2,0,1) lava aquí */
        (0,0,2),(1,0,2),               // (2,0,2) ausente para L
    ];
    for (x,y,z) in top_positions {
        add_block(&mut objects, x,y,z, grass_tex, Vec3::new(1.0,1.0,1.0),
                  grass_p.0, grass_p.1, grass_p.2, grass_p.3, grass_p.4, grass_p.5);
    }

    // Lava arriba a la derecha (2,0,1)
    add_block(&mut objects, 2,0,1, lava_tex, Vec3::new(1.0,1.0,1.0),
              lava_p.0, lava_p.1, lava_p.2, lava_p.3, lava_p.4, lava_p.5);

    // Capa media (y=-1): 3x3 de tierra
    for x in 0..3 {
        for z in 0..3 {
            add_block(&mut objects, x,-1,z, dirt_tex, Vec3::new(1.0,1.0,1.0),
                      dirt_p.0, dirt_p.1, dirt_p.2, dirt_p.3, dirt_p.4, dirt_p.5);
        }
    }
    ...
    for x in 1..3 {
        for z in 1..3 {
            add_block(&mut objects, x-1,-2,z-1, dirt_tex, Vec3::new(1.0,1.0,1.0),
                      dirt_p.0, dirt_p.1, dirt_p.2, dirt_p.3, dirt_p.4, dirt_p.5);
        }
    }

    // Bloque de piedra saliente en el frente para que se vea
    add_block(&mut objects, 0,-1,3, stone_tex, Vec3::new(1.0,1.0,1.0),
              stone_p.0, stone_p.1, stone_p.2, stone_p.3, stone_p.4, stone_p.5);

    // -------- Árbol ----------
    // Tronco (columna) en (0,*,1), desde y=1..3
    for y in 1..=3 {
        add_block(&mut objects, 0,y,1, wood_tex, Vec3::new(1.0,1.0,1.0),
                  wood_p.0, wood_p.1, wood_p.2, wood_p.3, wood_p.4, wood_p.5);
    }
    ...
    for y in 3..=4 {
        for dx in -1..=1 {
            for dz in -1..=1 {
                ...
                add_block(&mut objects, cx+dx, y, cz+dz,
                          leaves_tex, Vec3::new(1.0,1.0,1.0),
                          leaves_p.0, leaves_p.1, leaves_p.2, leaves_p.3, leaves_p.4, leaves_p.5);
            }
        }
    }
